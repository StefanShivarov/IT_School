...
public class OXPanel extends JPanel{
	...
	private void drawWinner(Graphics g) {
		...
		if(winningCombination == horizontalWin) {
			...
			g.drawLine(leftBorder(winningI) + squareWidth/2, upBorder(0), leftBorder(winningJ) + squareWidth/2, downBorder(2));
			...
			for(int j = 0; j < 3; j++) {
				if(board[i][j]=='x') {
					...
					g.drawOval((int) (leftBorder(j) + elementMarginRatio*squareWidth),
							(int) (upBorder(i) + elementMarginRatio*squareWidth),
							(int) (squareWidth - 2 * elementMarginRatio*squareWidth),
							(int) (squareHeight - 2 * elementMarginRatio*squareWidth));
				}
			}
		}
	...
	private int upBorder(int j) {
		return j*squareHeight + upBoardBorder;
	}
	
	private int downBorder(int j) {
		return (j+1)*squareHeight + upBoardBorder;
	}
}
